...
未来候选增强可以考虑引入统一的“可重试执行机制”，用于帮助某个显式声明可重试的过程在单次 workflow 运行内再次执行。

这里强调的是“统一执行机制”，不是“统一重试策略”。

也就是说：

- 框架层只负责“再次执行一次”
- 是否允许重试、最多尝试几次、失败后是否继续，仍由调用方显式声明
- 不允许把“任何失败都自动重试 N 次”做成默认行为

该机制仍应明确限定为“单次 workflow 运行内的自动重试”，不要和“resume 单个失败 task”混为一谈：
...
当前已知的业务约束应直接写死，不要过度抽象：

- 只有 `segments` 这个 LLM task 接入该机制
- 业务层对于 LLM 最多允许重复两次
- 因此 `segments.maxAttempts = 3`
- `subtitle`、`audio`、`slice` 当前都不接入
- `MissingSubtitleError` 仍然是预期内 `skipped`
- 其他错误默认直接失败，不做隐式兜底
...
- `maxAttempts`
- 一个由调用方提供的决策逻辑，用于决定失败后是否再次执行

这里的关键点是：

- 是否重试不由框架猜测
- 重试机制本身不吞错、不降级、不兜底
- 它只根据调用方给出的规则决定“再执行一次”还是“直接抛出”
...
如果最终 report 需要反映重试情况，则应把 `attemptCount` 暴露到对应 part 的 report 中。

对于当前已知场景，建议语义是：

- `attemptCount` 表示该 part 的 `segments` task 总尝试次数
- 最小值为 `1`
- 失败后又重跑两次，则 `attemptCount = 3`
- `short` 和 `missingSubtitle` 这类未进入 LLM task 的 skipped part，不要求暴露该字段
...
- `segments` 的单次失败不应立即让 part 进入 `failed`
- 只有 `segments` 用尽 3 次尝试后，part 才最终 `failed`
...
对于当前业务，`attemptCount` 的汇总也应保持简单：

- `part` 级只反映 `segments` 的尝试次数
- 不把 publish、merge 或其他阶段的失败混进这个字段
...
- 当前只有 `segments` 的 LLM 执行失败会进入 retry 机制
- 其他 task 即使失败，也默认直接失败

如果未来要把这个机制复用到其他阶段，例如 `git push`，也应由那个调用点显式声明自己的重试条件，而不是沿用 `segments` 的规则。
