...
export class ScboyVideoStore {
  ...
  async saveVideosForDateInTitle(
    client: Client,
    dateInTitle?: DateInTitleFilter,
  ): Promise<BiliVideo[]> {
    this.init();
    const dateRange = parseDateRange(dateInTitle);
    if (dateRange === null) {
      return this.saveLatestVideos(client);
    }

    let currentDelay = 1000;
    for (let page = 1; ; page += 1) {
      const pageVideosResult = await this.fetchVideoPage(
        client,
        page,
        currentDelay,
      );
      currentDelay = pageVideosResult.nextDelayMs;
      const oldestVideo = pageVideosResult.videos[pageVideosResult.videos.length - 1];
      if (
        oldestVideo === undefined ||
        oldestVideo.uploadAt.getTime() < dateRange[0].getTime() ||
        pageVideosResult.videos.length < 40
      ) {
        const videos = this.loadVideos();
        console.log(
          `saved videos for ${Array.isArray(dateInTitle) ? dateInTitle.join(" ") : dateInTitle}, total videos in db: ${videos.length}`,
        );
        return videos;
      }
    }
  ...
  async fetchVideoPage(
    client: Client,
    page: number,
    currentDelayMs: number,
  ): Promise<{ videos: BiliVideo[]; nextDelayMs: number }> {
    ...
    for (;;) {
      try {
        ...
        console.log(`page ok ${page}, current delay: ${currentDelayMs / 1000}s`);
        const nextDelayMs = Math.max(1000, Math.floor(currentDelayMs * 0.8));
        await Bun.sleep(nextDelayMs);
        return { videos, nextDelayMs };
      } catch (error) {
        ...
        if (isRateLimitError(error)) {
          console.log(
            `Rate limit hit at page ${page}, waiting ${currentDelayMs / 1000}s before retry`,
          );
          await Bun.sleep(currentDelayMs);
